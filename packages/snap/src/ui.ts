...
function prettyPrintOrderSubmission(tx: any, textFn: any) {
    let elms = [];
    let isLimit = tx.price !== undefined && tx.price !== null && tx.price !== '';
    let side = tx.side == 'TYPE_BUY' ? 'buy' : 'sell';

    if (tx.peggedOrder) {
	elms.push(textFn(`Pegged Limit ${side} - ${getTimeInForce(tx.timeInForce)} ${tx.size} @ ${getPeggedReference(tx.peggedOrder.reference)}+${tx.peggedOrder.offset}`))
    } else if (isLimit) {
	elms.push(textFn(`Limit ${side} - ${getTimeInForce(tx.timeInForce)} ${tx.size} @ ${tx.price}`))
    } else {
	elms.push(textFn(`Market ${side} - ${getTimeInForce(tx.timeInForce)} ${tx.size}`))
    }
	...
	elms.push(textFn(`**iceberg minimum visible size**: ${tx.icebergOpts.minimumVisibleSize}`));
    }
...
function prettyPrintOrderAmendment(tx: any, textFn: any) {
    ...
    if (tx.sizeDelta !== undefined && tx.sizeDelta !== null && tx.sizeDelta !== 0) {
	if (tx.sizeDelta > 0) {
    ...
    if (tx.expiresAt !== undefined && tx.expiresAt !== null && tx.expiresAt !== 0) {
	elms.push(textFn(`**expires at**: ${tx.expiresAt}`))
    }
    if (tx.timeInForce !== undefined && tx.timeInForce !== null && tx.timeInForce !== '') {
	elms.push(textFn(`**time in force**: ${getTimeInForce(tx.timeInForce)}`))
    }
    if (tx.peggedReference !== undefined && tx.peggedReference !== null && tx.peggededReference !== '') {
	elms.push(textFn(`**pegged reference**: ${getPeggedReference(tx.peggedReference)}`))
    }
    if (tx.peggedOffset !== undefined && tx.peggedOffset !== null && tx.peggededOffset !== '') {
...
function prettyPrintCancelOrder(tx: any, textFn: any) {
    const hasOrderId = tx.orderId !== undefined && tx.orderId !== null && tx.orderId != '';
    const hasMarketId = tx.marketId !== undefined && tx.marketId !== null && tx.marketId != '';
